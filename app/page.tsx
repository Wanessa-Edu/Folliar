...
/* ── HELPERS ── */
function Pill({ch,t="p-sage"}:{ch:ReactNode;t?:string}){return <span className={"pill "+t}>{ch}</span>}
function H({tag,title,desc}:{tag:ReactNode;title:ReactNode;desc?:ReactNode}){
  return(
    <div>
...
function Card({title,sub,ch,style,glow}:{title?:ReactNode;sub?:ReactNode;ch:ReactNode;style?:CSSProperties;glow?:boolean}){
  return(
...
function FR({icon,name,desc}:{icon:ReactNode;name:ReactNode;desc:ReactNode}){
  return(
    <div className="fr">
...
function Screens(){
  ...
  type ScreenId = (typeof LIST)[number]["id"]
  type ScreenData = {
    goal: string
    components: string[]
    ux: string
  ...
  const [sc,setSc]=useState<ScreenId>("dashboard")
  const DATA={
    landing:{
  ...
  } satisfies Record<ScreenId, ScreenData>
  const s=DATA[sc]
  return(
    <div>
...
/* ══════════════════════════════════════════
   APP SHELL
══════════════════════════════════════════ */
const VIEWS: Record<ViewId, ComponentType> = {brand:Brand,screens:Screens,features:Features,tech:Tech,db:DB,roadmap:Roadmap,pitch:Pitch}

export default function App(){
  const [active,setActive]=useState<ViewId>("brand")
